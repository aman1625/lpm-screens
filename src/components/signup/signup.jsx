...
const SignUp = () => {
    ...
    const passwordLength = (password)=> password.length >= 8 ? true : false

    const isUppercase = (password)=>/[A-Z]/.test(password) ? true : false
    
    const isOneNumber = password => /[0-9]/.test(password) ? true : false
    
    const isSpecialCharacter= password => /[!@#$%^&*)(+=._-]/.test(password) ? true : false
    ...
    const onPasswordChange = (e)=>{
        let input = e.target.value;
        setCheckBoxes({
            isLengthy : (passwordLength(input)),
            hasUppercase : (isUppercase(input)),
            hasNumber: (isOneNumber(input)),
            hasSpecialCharacter : (isSpecialCharacter(input))
        })
        setUserCredentials(prev=>({
            ...prev,
            password : input
                ...
                <div className='inputs w-full flex flex-col justify-center items-center space-y-6' >
                    ...
                    <div className='w-11/12 sm:w-3/4 md:w-1/2 flex flex-col justify-center items-center text-left' >
                        ...
                        <input name='email' type='email' className=' text-sm px-1 py-2 bg-transparent w-full border-b border-gray-1000 focus:outline-none focus:border-gray-500 ' autoComplete='off' placeholder='john.doe@alphametics.com'
                          onChange={onEmailChange} value={userCredentials.email}
                         />
                    ...
                    <div className='w-11/12 sm:w-3/4 md:w-1/2 flex flex-col justify-center items-center text-left' >
                        ...
                        onChange={onPasswordChange} value={userCredentials.password}
                        />
                    </div>
                    <div className='w-11/12 sm:w-3/4 md:w-1/2 flex flex-col justify-center items-center text-left' >
                        ...
                        <input name='confirm-password' type='password' className='tracking-widest text-2xl  px-1 py-2 bg-transparent w-full border-b border-gray-1000 focus:outline-none focus:border-gray-500 ' autoComplete='off' 
                            onChange={onConfirmPasswordChange} value={userCredentials.confirmPassword}
                        />
